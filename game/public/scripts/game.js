...
function displayChoices(choices) {
    ...
    choices.forEach(choice => {
        ...
        effectsDiv.innerHTML = `
            ...
            ${choice.effects.inventory && choice.effects.inventory.add ? `<p>Gain: ${Object.entries(choice.effects.inventory.add).map(([item, quantity]) => `${quantity} ${item}`).join(', ')}</p>` : ''}
            ${choice.effects.inventory && choice.effects.inventory.remove ? `<p>Lose: ${Object.entries(choice.effects.inventory.remove).map(([item, quantity]) => `${quantity} ${item}`).join(', ')}</p>` : ''}
        `;
...
async function takeAction(action) {
    try {
        ...
        currentGameState = gameUpdate.gameState; // Ensure this is a full game state object
